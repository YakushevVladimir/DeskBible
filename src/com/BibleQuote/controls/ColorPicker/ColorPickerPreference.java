...
package com.BibleQuote.controls.ColorPicker;
...
public class ColorPickerPreference extends Preference
	...
	protected void onSetInitialValue(boolean restoreValue, Object defaultValue) {
		onColorChanged(restoreValue ? getValue() : convertToColorInt((String) defaultValue));
	}
		...
		if (attrs != null) {
			...
			if (defaultValue.startsWith("#")) {
				try {
					...
					mDefaultValue = "#FF000000";
				}
			} else {
				int resourceId = attrs.getAttributeResourceValue(androidns, "defaultValue", 0);
				if (resourceId != 0) {
					mDefaultValue = context.getResources().getString(resourceId);
				}
			}
	...
	public int getValue() {
		...
		return convertToColorInt(mValue);
	}

	@Override
	public void onColorChanged(int color) {
		String newColor = convertToARGB(color);
		if (isPersistent()) {
			persistString(newColor);
		}
		mValue = newColor;
		setPreviewColor();
		try {
